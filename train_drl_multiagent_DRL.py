...
def train_multi_agent(s: str, l: float):
    ...
    expected_files = [f"critic_{p.name}.pt" for p in processes] + [f"actor_{p.name}.pt" for p in processes]
    ...
    if resume_episode > 0:
        ...
        for pname, agent in agent_proc.agents.items():
            critic_path = os.path.join(CHECKPOINT_DIR, f"critic_{pname}_ep{resume_episode}.pt")
    ...
    for episode in trange(resume_episode, MAX_EPISODES, desc=f"Training {s} λ={l}"):
        ...
        avg_cycle_time = float(gact_table.at[("mean_gact", l), "TrainEval"])
        ...
        if this_score < best_score - 1e-9:
            ...
            print(f"[BEST] ↓ improved best per-process score to {this_score:.4f} at episode {episode}. Saved best actor/critic (all processes).")
    ...
    for pname, agent in agent_proc.agents.items():
        torch.save(agent.critic.state_dict(), os.path.join(SAVE_DIR, f"critic_{pname}_last.pt"))
        torch.save(agent.actor.state_dict(),  os.path.join(SAVE_DIR, f"actor_{pname}_last.pt"))
    if not converged:
