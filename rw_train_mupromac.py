...
def train_mupromac(s: str, l: float):
    ...
    for episode in trange(resume_episode, MAX_EPISODES, desc=f"Training {s} λ={l}"):
        ...
        avg_ct, avg_ct_by_proc, thr_by_proc, total_thr = compute_episode_metrics(df, l, "TrainEval")
        ...
        if this_score < best_score - 1e-9:
            ...
            print(f"[BEST] ↓ improved best score to {this_score:.4f} at episode {episode}.")

        if (episode + 1) % CHECKPOINT_INTERVAL == 0:
            torch.save(mupromac_process.agent.critic.state_dict(), os.path.join(CHECKPOINT_DIR, f"critic_ep{episode + 1}.pt"))
            for pname, actor in mupromac_process.agent.actors.items():
