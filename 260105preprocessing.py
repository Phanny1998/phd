...
EVENT_OUT_DIR = os.path.join(OUT_ROOT, "event_logs_processed")
...
def prepare_output_format(events: pd.DataFrame) -> pd.DataFrame:
    """
    Select and order columns to match Verenich pipeline expectations.
    Column names must match dataset_confs.py!
    """
    output_cols = [
        "case_id",
        "activity",
