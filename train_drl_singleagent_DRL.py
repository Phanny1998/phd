...
def train_single_agent(scenario_name: str, lam: float):
    ...
    for episode in trange(resume_episode, MAX_EPISODES, desc=f"Training {scenario_name} λ={lam}"):
        ...
        avg_cycle_time = gact_table.at[("mean_gact", lam), "TrainEval"]
        ...
        if this_score < best_score - 1e-9:
            ...
            print(f"[BEST] ↓ improved best per-process score to {this_score:.4f} at episode {episode}. Saved best actor/critic.")

        if (episode + 1) % CHECKPOINT_INTERVAL == 0:
            torch.save(
                agent_proc.agent.critic.state_dict(),
                os.path.join(CHECKPOINT_DIR, f"critic_ep{episode + 1}.pt")
            )
            torch.save(
                agent_proc.agent.actor.state_dict(),
                os.path.join(CHECKPOINT_DIR, f"actor_ep{episode + 1}.pt")
            )
...
if __name__ == "__main__":
    ...
    with Pool(processes=min(12, len(args_list))) as pool:
